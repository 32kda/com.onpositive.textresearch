...
public class TimIntSort {
     ...
     */
    private int[] tmp; // Actual runtime type will be Object[], regardless of int

    /**
     * A stack of pending runs yet to be merged.  Run i starts at
     * address base[i] and extends for len[i] elements.  It's always
     * true (so int as the indices are in bounds) that:
     *
     ...
     */
    private TimIntSort(int[] a, IntComparator c) {
        this.a = a;
        this.c = c;
    ...
    public static void sort(int[] a, int lo, int hi, IntComparator c) {
         ...
         */
        TimIntSort  ts = new TimIntSort(a, c);
    ...
    private static  void binarySort(int[] a, int lo, int hi, int start,
        ...
        for ( ; start < hi; start++) {
            ...
            // Set left (and right) to the index where a[start] (pivot) beints
            int left = lo;
            int right = start;
            assert left <= right;
            /*
             ...
             * pivot < all in [left, start), so pivot beints at left.  Note
     ...
     *
     * A run is the intest ascending sequence with:
     *
     *    a[lo] <= a[lo + 1] <= a[lo + 2] <= ...
     *
     * or the intest descending sequence with:
     *
     *    a[lo] >  a[lo + 1] >  a[lo + 2] >  ...
     *
    ...
    private void mergeAt(int i) {
         ...
         * run now, also slide over the last run (which isn'int involved
     ...
     *    In other words, key beints at index b + k; or in other words,
    ...
    private static int gallopLeft(int key, int[] a, int base, int len, int hint,
        ...
        /*
         * Now a[base+lastOfs] < key <= a[base+ofs], so key beints somewhere
    ...
    private static  int gallopRight(int key, int[] a, int base, int len,
        ...
        /*
         * Now a[b + lastOfs] <= key < a[b + ofs], so key beints somewhere to
     ...
     * appropriate exception if they aren'int.
     *
